...
const ROUTE_SECTION_RULES: Array<{ pattern: RegExp; section: SyllabusSectionNumber }> =
  [
    { pattern: /\/datos-generales\b/i, section: SYLLABUS_SECTION.DATOS_GENERALES },
    ...
    {
      pattern: /\/(estrategias_metodologicas|estrategias|methodological-strategies)\b/i,
    ...
    {
      pattern: /\/(formula_evaluacion|formula|evaluacion_aprendizaje|plan_evaluacion)\b/i,
...
export function getSectionLabel(sectionNumber: number): string {
  return (
    SECTION_EDIT_LABELS[sectionNumber] ?? `sección ${sectionNumber}`
  );
}

/** Resuelve número de sección a partir del path de la petición (p. ej. /api/syllabus/10/competencies). */
export function getSectionNumberForPath(path: string): SyllabusSectionNumber | null {
...
export function getSectionNumberForEndpoint(
  ...
  const key = String(endpointKey ?? "").trim().toLowerCase();
