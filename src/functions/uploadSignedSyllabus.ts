...
// File handling
const generateSafeFileName = (originalName: string, silaboId: number): string => {
...
app.http("director_upload_signed_syllabus", {
  ...
  handler: async (req) => {
    try {
      ...
      // Save to database
      await saveSignedSyllabus(db, silaboId, ciclo, validatedFile.name, filePath);
      ...
      const statusCode = errorMessage.includes("seleccionar") || errorMessage.includes("válida") || errorMessage.includes("encontrado") ? 400 : 500;
